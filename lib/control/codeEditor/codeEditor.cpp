...
control::codeEditor::codeEditor(QWidget *parent, QString syntax) : QPlainTextEdit(parent)
{
    ...
    if (syntax == "llvm")
    {
        highlighter = new control::syntax::llvmHighlighter(this->document());
        lineNumberArea = new LineNumberArea(this, true);
    }
    if (syntax == "markdown")
    {
        highlighter = new control::syntax::MarkdownHighlighter(this->document());
        lineNumberArea = new LineNumberArea(this, false);
    }
    else
        highlighter = new control::syntax::clangHighlighter(this->document());
